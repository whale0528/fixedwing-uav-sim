...
**待办/开放问题**
- [x] MATLAB 会话连接本工作环境（MCP 桥接），以便直接协助调试 —— 已接通（MATLAB R2025b + MATLAB MCP，`connector on` 后可用）
- [x] 确认现有 Simulink 模型结构（6DOF + 制导律 + 控制器）—— 已确认：`b0307.mdl` = 6DOF 平动/转动动力学 + 风场 + BTT + 巡航段/末制导 + 俯仰过载与滚转姿态控制器 + 带 Dubins 圆弧的航路跟踪；模型从 base 工作区读 `fly_pt` 航点表
- [x] 选 LLM API（DeepSeek 性价比高；需处理网络与 key）—— 已用 DeepSeek（`llm_key.txt` 存 key、已 gitignore；接口连通实测 0.8 s）
- [x] 决定方案 A（航点）还是方案 B（standoff 直给制导律）作为首版 —— 采**方案 A**（航点）；且因模型圆弧切换不可靠，最终改为**全直线离散 + 正多边形绕圈**（见下）

---

## 6. 实现状态（2026-09-14 更新）

**路线二已落地并实测通过**（代码在本仓库根目录）：

| 环节 | 实现 | 说明 |
|---|---|---|
抽取 | `llm2route.m` + `parse_llm_json.m` | DeepSeek，temperature=0，强制 JSON，3 次重试 |
校验（校验点①） | `check_route_spec.m` | 地标查表、**禁 LLM 生成坐标**、半径/圈数/距离钳制、段数≤12、默认值注入 + issues |
轨迹生成 | `make_route_plan.m`（通用动作原语）+ `make_orbit_plan.m`（绕圈封装） | 6 种原语：turn_to/line/line_to/goto/orbit/exit_line；所有弧段离散成直线航点 |
仿真执行 | `run_route_sim.m` / `run_orbit_sim.m` | 自动按路径长度定时长，任务完成即停 |
人工确认（HITL） | `confirm_route.m` + `route2nl.m` | LLM 把计划回译成人话 + 确定性清单 + 命令行 y/n；离线自动回退 |
执行判定（校验点②） | `check_route_flight.m` / `check_orbit_flight.m` | 到点误差、圈数、半径 RMSE |
一键入口 | `run_llm_mission.m` | 一句话 → 抽取 → 校验 → 航点 → 确认 → 仿真 → 判定 |

**实测结论**：
- 说法鲁棒性清单 16 条全部通过（含中文数字"十圈"、公里换算、"再回来"→返航、坐标说法转最近地标、超范围钳制、未知地标/不支持动作拒绝）；
- 端到端："向北飞2000米以后绕训练空域中心转3圈再回来" → 175 航点、25.4 km、仿真 799 s，绕圈实际 3.22 圈、半径 RMSE 22 m；
- 检查项合计 47 项（航点 12 + 校验 15 + 抽取 14 + 确认 4 + 判定 2），`run_tests` 一键运行；另有 16 条说法鲁棒性清单（`tests/test_llm_phrasing.m`）；
- 模型 `b0307.mdl` 仅改 1 个参数（`kdy` 0.005→0.02，解锁更小半径），结构零改动。

**仍未做**：
- 路线一（PDDL/TSP 神经符号规划）—— 任务集里暂无依赖/资源/多机需求，暂不引入；
- 多轮消歧（缺参反问）、执行失败 → 回 LLM 重规划（闭环）；
- 变速/增益调度、standoff 制导律（方案 B）。
