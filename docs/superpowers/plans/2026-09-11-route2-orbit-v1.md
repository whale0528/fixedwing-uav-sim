...
## LLM 层实现与实测（2026-09-14）

**与计划的偏差**：计划里的任务 5/6/7 是按"绕圈专用"设计的；实际执行时先做了一步**通用化重构**（`make_route_plan` 段原语 + `run_route_sim`），校验与抽取都改为面向"动作单"，否则校验层写完就要返工。

| 计划任务 | 实际实现 | 检查项 |
|---|---|---|
| 前置重构 | `make_route_plan.m`（6 种原语）+ `run_route_sim.m`；`make_orbit_plan` 变薄封装 | 12 项（含新旧输出逐位一致） |
| 任务 5 校验层 | `check_route_spec.m`：地标查表、禁坐标、半径/圈数/距离钳制、段数≤12、默认值注入 | 15 项 |
| 任务 6 抽取层 | `llm2route.m` + `parse_llm_json.m`（DeepSeek、temperature=0、强制 JSON、3 次重试） | 14 项（6 离线 + 8 在线） |
| 任务 7 端到端 | `run_llm_mission.m` + `check_route_flight.m`（通用判定） | 在线实飞验证 |
| （计划外）HITL | `confirm_route.m` + `route2nl.m`：LLM 回译 + 确定性清单 + 命令行 y/n，离线可回退 | 4 项 |
| 任务 8 鲁棒性 | `tests/test_llm_phrasing.m`：16 条说法清单 | 16 项 |

**说法鲁棒性实测（16/16）**：中文数字（"十圈"→10）、单位换算（公里→米）、口语（"再回来"→返航到起飞点）、模糊指令（缺参→默认值+提示）、超范围（100 圈→50、半径 100 m→400 m）、非法（未知地标/不支持动作→空动作单并拒绝）均处理正确。

**本轮修掉的三个真实问题：**
1. **静默替换**：用户给坐标（"北3000东2000那里"）时，LLM 悄悄换成别的地标且无任何提示 → prompt 新增规则：必须选**最接近**的地标并在 `assumptions` 声明"用户给的是坐标"；
2. **不支持动作**：如"飞个八字"，行为不确定 → prompt 新增规则：返回**空动作单 + 说明**，不许编造动作；
3. **字符串健壮性**：`string(NaN)` 在 MATLAB 返回 `<missing>`，会让校验层在读到空/缺失字段时报错 → 新增 `safestr()` 统一安全转换，并给 goto 段补上坐标越界致命检查。

**检查项合计 47 项**（航点 12 + 校验 15 + 抽取 14 + 确认 4 + 判定 2），`run_tests` 一键运行；另有 16 条说法鲁棒性清单（`tests/test_llm_phrasing.m`）。模型仅改 `kdy` 一个参数。

**阶段二仍未做**：多轮消歧（缺参反问）、执行失败 → 回 LLM 重规划、standoff 制导律（方案 B）、变速/增益调度；路线一（PDDL/TSP）整体未动。
